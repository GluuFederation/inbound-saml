...
describe('getFromFile', () => {
  it('should return correct values', async () => {
    const api = new Getter()
    const result = await api.getFromFile(validFilePath)
    const expected = {
      requestId: '88299e70-23dd-4f90-86ab-46b40122f467',
      response: {
        externalData: {
          idpSigningCert: [
    ...
    expect(result.response).toStrictEqual(expected.response)
  })
})
