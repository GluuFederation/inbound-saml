...
jest.mock(
  '@get-saml-metadata/interface-adapters/data/adapters/XmlValidatorAdapter'
)
...
describe('MetadataMapperAdapter', () => {
  describe('map', () => {
    it('should call getIdpssoDescriptor with correct params', () => {
      ...
      const getIdpssoDescriptorSpy = jest
        .spyOn(MetadataMapperAdapter.prototype as any, 'getIdpssoDescriptor')
  ...
  describe('getIdpssoDescriptor', () => {
    it('should call fast-xml-parser parse with correct params', () => {
      const parseSpy = jest
        .spyOn(parser, 'parse')
        .mockReturnValueOnce(parsedMetadataString)
      const sut = new MetadataMapperAdapter()
